...
const ProductFixed = () => {
    const { img , price , saleOptIn , length , accessType ,certification } = courses[0]
    ...
    //Classes 
    const fixedClass = tooClose  ? 'product-cta__buynow' : 'product-cta__buynow--offset'
        ...
        <div className = {fixedClass}>
           ...
           <div className = 'product-cta__buynow--bottom'>
               <div className = 'product-cta__buynow--bottom--price'>${currentPrice}
                    {saleOptIn && sale  ? <span className = 'product-cta__buynow--bottom--price--slash'>${price}</span> : null}
               </div>

               <input className = 'product-cta__buynow--bottom--addtocart'
               type = 'submit'
               value ='add to cart' />
