...
const {ErrorHandler} = require('../frontend/src/utils/ErrorHandler')
...
exports.signup = async (req ,res ,next) => {
    try{
        ...
        if(password.length < 8){
            return next(new ErrorHandler(401,'Passwords must be at least'))
        }
        const pw = await encryptPW(password)
        const newUser = await User.create({ email , password :pw})
